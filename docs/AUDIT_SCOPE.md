...
| `lib/script/interpreter.js` | 2,834 | **Scoped to the flag and era surface, not opcode execution.** Consensus-flag selection and defaults, era derivation (Genesis/Chronicle), the limits derived from them, and the semantics of the exported `verify()`. Opcode execution is excluded — see §2.2. |
...
Totals reconcile against `lib/`, which is 39,673 lines across 131 files:
...
Scope, and we would like these priced separately:
  ...
  Tier 1 — 11,989 lines. Sighash construction and signing; ECDSA, nonce
