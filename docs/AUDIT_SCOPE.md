...
| --- | ---: | --- |
| `lib/transaction/` | 2,786 | Sighash construction and signing — both BIP-143 and the Original Transaction Digest Algorithm. |
| `lib/script/interpreter.js` | 2,851 | **Scoped to the flag and era surface, not opcode execution.** Consensus-flag selection and defaults, era derivation (Genesis/Chronicle), the limits derived from them, and the semantics of the exported `verify()`. Opcode execution is excluded — see §2.2. |
...
Totals reconcile against `lib/`, which is 39,748 lines across 131 files:
...
Scope, and we would like these priced separately:
  ...
  Tier 1 — 12,013 lines. Sighash construction and signing; ECDSA, nonce
