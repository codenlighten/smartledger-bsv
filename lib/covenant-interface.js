...
class CovenantInterface {
  /**
   * Convenience access to the assembled library, for consumers who have a
   * CovenantInterface and want the rest of the namespace.
   *
   * A getter, not a constructor assignment. The previous comment justified the
   * deferral with "this file is reached while index.js is still initializing" —
   * which is not true: nothing in index.js or anywhere under lib/ requires this
   * module, only two files in examples/. It was also never read internally, so an
   * eager require bought nothing and left this as the last package-root require in
   * lib/ after a refactor whose whole purpose was removing them.
   *
   * Resolving on access keeps the property working for anyone using it while making
   * the root require cost nothing for everyone else. `require` is cached.
   */
  get bsv () {
  ...
  getPreimage (transaction, inputIndex, lockingScript, satoshis, sighashType = null) {
    ...
    const preimage = sighash.sighash(
      transaction,
      sighashType,
      inputIndex,
      lockingScript,
  ...
  createTokenTransfer (newOwner, minAmount, additionalRules = null) {
    ...
    if (minAmount > 0) {
      ...
      script.add(Opcode.OP_GREATERTHANOREQUAL)
      script.add(Opcode.OP_VERIFY)
    }
  ...
  createPushtxCovenant (params = {}) {
    ...
    script.add(Opcode.OP_2DUP)
      .add(Opcode.OP_HASH256) // Hash outputs (item 9)
      .add(Opcode.OP_SWAP)
    ...
    script.add(Opcode.OP_CHECKSIGVERIFY)
  ...
  _addPreimageConstruction (script, params) {
    ...
    script.add(Buffer.from(sequence.toString(16).padStart(8, '0'), 'hex'))
      .add(Opcode.OP_CAT)
      .add(Opcode.OP_SWAP) // hashOutputs already computed
      .add(Opcode.OP_CAT)
