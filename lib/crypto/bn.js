...
BN.prototype.toSMBigEndian = function () {
  ...
  // `&` here was a bitwise AND between two booleans. It happened to work because
  // `===` binds tighter than `&` and `true & true` is 1, but the intent is logical.
 ...
 *
 * Script number decoding fails for two consensus reasons, and both are named
 * in the node's script vectors — SCRIPT_ERR_SCRIPTNUM_OVERFLOW and
 * SCRIPT_ERR_SCRIPTNUM_MINENCODE. This layer knows which one it hit, and the
 * interpreter's evaluator is where that has to be reported, so the code
 * travels on the error rather than being guessed from its message. An error
 * without a `scriptErr` really is unknown to the interpreter and is still
 * reported as such.
 */
function scriptNumError (message, scriptErr) {
