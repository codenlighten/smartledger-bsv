...
// NOTE: verify() returns the ECDSA *instance* (for chaining, e.g.
// `.sign().verify()`), NOT a boolean. The pass/fail result is on `.verified`.
// Treating the return value as a boolean is ALWAYS truthy and silently accepts
// forged signatures — use `.verify().verified` or the `verifyBool()` helper.
...
/**
 * Boolean verification — the safe form. Returns `true` only when the signature
 * is valid, avoiding the truthy-instance trap of verify().
 * @returns {Boolean}
 */
ECDSA.prototype.verifyBool = function () {
  return this.verify().verified === true
}
