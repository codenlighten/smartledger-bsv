...
// The browser digests come from @noble/hashes — the same audited implementation the
// Node path's primitives ultimately rest on, and already a dependency here for
// @noble/curves. Previously this file used `hash.js`, so the browser bundle and Node
// computed the same digests via two different implementations; one of them was audited.
//
// Only the four raw digests came from that dependency. `Hash.hmac` below is
// hand-implemented against `hashf.blocksize` and is unchanged.
var { sha256, sha512 } = require('@noble/hashes/sha2.js')
var { sha1, ripemd160 } = require('@noble/hashes/legacy.js')
...
Hash.hmac = function (hashf, data, key) {
  ...
  if (key.length > blocksize) {
    ...
    // Was `key < blocksize` — a Buffer compared against a number, which is always
    // false, so this zero-padding branch never ran. The result was correct anyway:
    // the XOR loop below indexes past a short key and `x ^ undefined === x ^ 0`,
    // which is exactly what the padding produces. Fixed so the code says what it
    // does; HMAC output is unchanged, and the vectors below assert that.
