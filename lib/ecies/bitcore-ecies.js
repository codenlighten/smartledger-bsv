...
// Constant-time buffer comparison. Works in Node and browser bundles alike
// (crypto.timingSafeEqual is not provided by crypto-browserify). Compares the
// full length unconditionally so timing does not leak how many bytes matched.
...
ECIES.prototype.decrypt = function (encbuf) {
  ...
  // Constant-time MAC comparison (encrypt-then-MAC verification).
  if (!constantTimeEqual(d, d2)) {
