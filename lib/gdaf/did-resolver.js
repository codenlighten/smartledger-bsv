...
DIDResolver.verifyOwnership = function(did, signature, message) {
  try {
    ...
    // ECDSA.prototype.verify() returns the instance (always truthy); the boolean
    // result lives on .verified. Returning the instance would accept ANY signature.
    return ecdsa.verify().verified === true
  } catch (e) {
    return false
  }
}
