...
SmartLedgerAnchor.verifyAnchor = async function(txid, expectedHash, options) {
  ...
  if (typeof options.chainProvider !== 'function') {
    throw new Error(
      'verifyAnchor requires options.chainProvider(txid) backed by a trusted BSV ' +
      'source (SPV / full node / explorer) returning { confirmations, blockHeight, ' +
      'opReturns }. Refusing to fabricate a verification result.'
    )
  }
