...
var LTPAnchor = {
  ...
  /**
   * Verify that CALLER-SUPPLIED transaction data commits to the token.
   *
   * ⚠️ This does NOT prove the token is anchored on-chain. It only checks that the
   * OP_RETURN in the `txData` you pass matches the token's commitment. It performs
   * NO chain query, NO SPV/inclusion proof, and does NOT confirm `txData` actually
   * corresponds to `txid` or to a mined transaction. An attacker can fabricate
   * `txData` with a matching OP_RETURN and get `valid:true`. Treat `valid` as
   * "commitment matches the supplied data" only; obtain `txData` from a TRUSTED
   * source and verify inclusion separately (SPV / trusted node) before relying on it.
   * `chainVerified` is always false here to make that explicit.
   *
   * @param {Object} token - Token to verify
   * @param {String} txid - Transaction ID from external anchor
   * @param {Object} txData - Transaction data (must come from a trusted source)
   * @returns {Object} { valid, chainVerified:false, ... }
   */
  verifyTokenAnchor: function(token, txid, txData) {
    ...
    try {
      ...
      return {
        ...
        // Commitment-match only — NOT an on-chain inclusion proof. The caller must
        // verify txData came from a trusted source / SPV before trusting this.
        chainVerified: false,
        txid: txid,
