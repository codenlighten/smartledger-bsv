...
var ClaimValidator = {
  ...
  prepareClaimValidation: function(claim, schemaName) {
    try {
      ...
      var validation = {
        ...
        warnings: warnings,
        schema: schemaName,
        // Both pinned to LEGACY, not inheriting the deprecated default: these are
        // stored as identifiers by callers who cannot pass an argument here, and a
        // warning naming a function they did not call is noise. 10.0.0 moves them to
        // JCS as one documented change rather than a silent one.
        claimHash: Hash.sha256(Buffer.from(
          this._canonicalizeWith(claim, this.CANONICALIZATION.LEGACY)
        )).toString('hex'),
        canonical: this._canonicalizeWith(claim, this.CANONICALIZATION.LEGACY)
      }
  ...
  prepareClaimAttestation: function(claim, schemaName, attestor) {
    try {
      ...
      // Pinned to LEGACY — see the note on validateClaim's canonical field.
      var attestationHash = Hash.sha256(Buffer.from(
        this._canonicalizeWith(attestation, this.CANONICALIZATION.LEGACY)
      )).toString('hex')
  ...
  prepareClaimDispute: function(claimHash, disputant, dispute) {
    try {
      ...
      // Create dispute hash
      // Pinned to LEGACY — see the note on validateClaim's canonical field.
      var disputeHash = Hash.sha256(Buffer.from(
        this._canonicalizeWith(disputeRecord, this.CANONICALIZATION.LEGACY)
      )).toString('hex')
  ...
  /**
   * Canonicalization forms understood when hashing a claim.
   *
   * JCS is RFC 8785 and is what a claim hash should be computed with: two independent
   * implementations produce identical bytes, which is the only property that makes a
   * hash meaningful to anyone but us.
   *
   * LEGACY is the sorted-key form this module has always used. It sorts keys and then
   * REBUILDS an object, which loses the sort — V8 orders integer-like own properties
   * numerically ahead of string keys whatever order they were inserted in:
   *
   *     LEGACY   {"2":"two","10":"ten"}
   *     JCS      {"10":"ten","2":"two"}      (RFC 8785, UTF-16 code unit order)
   *
   * Deterministic, so nothing already hashed is wrong or forgeable. Not interoperable,
   * so an RFC 8785 verifier in any other language computes a different claim hash.
   */
  CANONICALIZATION: {
    JCS: 'jcs',
    LEGACY: 'legacy'
   ...
   * @param {String} [canonicalization] - CANONICALIZATION.JCS or .LEGACY. Omitting it
   *        selects LEGACY and emits a deprecation notice; the default becomes JCS in
   *        10.0.0. Pass one explicitly to pin the behaviour across that change.
   * @returns {String} Canonical JSON string
   */
  canonicalize: function(claim, canonicalization) {
    return this._canonicalizeWith(
      claim,
      this._resolveCanonicalization(canonicalization, 'LTP.Claim.canonicalize')
    )
  },
   ...
   * @param {String} [canonicalization] - see canonicalize()
   * @returns {String} SHA256 hash
   */
  hash: function(claim, canonicalization) {
    var canonical = this._canonicalizeWith(
      claim,
      this._resolveCanonicalization(canonicalization, 'LTP.Claim.hash')
    )
    return Hash.sha256(Buffer.from(canonical)).toString('hex')
  },

  /**
   * Canonicalize with the form already decided. No deprecation notice: the caller has
   * chosen, either explicitly or on its behalf by _resolveCanonicalization.
   */
  _canonicalizeWith: function(claim, canonicalization) {
    $.checkArgument(claim && typeof claim === 'object', 'Invalid claim')

    if (canonicalization === this.CANONICALIZATION.JCS) {
      return JCS.stringify(claim)
    }
  ...
  /**
   * Resolve an optional canonicalization argument to a concrete form.
   *
   * An unrecognised value throws rather than falling back. A silent fallback would
   * turn a typo such as `'JCS'` into a differently-hashed claim that still looks
   * successful, which is the failure mode this whole change exists to remove.
   */
  _resolveCanonicalization: function(canonicalization, caller) {
    if (canonicalization === undefined) {
      // Per STABILITY.md: mark it in a minor, flip it in a major. 9.x does not break
      // callers, so the default stays LEGACY and the notice carries the migration.
      deprecate({
        what: caller + '() without a canonicalization argument',
        since: '9.2.0',
        removeIn: '10.0.0',
        use: caller + '(claim, bsv.LTP.Claim.CANONICALIZATION.JCS)',
        why: 'the default sorted-key form is not RFC 8785, so other implementations compute a different claim hash'
      })
      return this.CANONICALIZATION.LEGACY
    }

    $.checkArgument(
      canonicalization === this.CANONICALIZATION.JCS ||
      canonicalization === this.CANONICALIZATION.LEGACY,
      'Unknown canonicalization: ' + canonicalization +
      ". Expected 'jcs' or 'legacy' (bsv.LTP.Claim.CANONICALIZATION)"
    )
