...
LTP.prototype.createRightToken = function(rightData, privateKey, options) {
  ...
  try {
    ...
    // Add cryptographic proof. Sign via the right-token signer (which sets
    // proof.jws + tokenHash); the older LTPProof.createSignature entrypoint no
    // longer exists (proof.js exposes prepareSignatureProof).
    if (options.addProof !== false) {
      token = LTPRight._signToken(token, privateKey)
    }
...
LTP.prototype.createSelectiveDisclosure = function(token, revealedFields, nonce) {
  return LTPProof.prepareSelectiveDisclosure(token, revealedFields, nonce)
}
...
LTP.prototype.verifyToken = function(token, publicKey) {
  ...
  // Instance-created tokens (right/transfer/obligation) are signed by
  // RightToken._signToken (detached ES256K JWS + tokenHash), so verification
  // goes through the matching RightToken.verifyToken rather than the
  // proofValue-based LTPProof path.
  return LTPRight.verifyToken(token, publicKey)
}
...
LTP.prototype.transferRight = function(token, newOwner, ownerKey, options) {
  ...
  try {
    // Verify current ownership against the owner's key before transferring.
    ...
    // Build and sign the transferred token. prepareRightTokenTransfer signs
    // the new token with the current owner's key, so no separate proof step
    // is needed.
    var transferResult = LTPRight.prepareRightTokenTransfer(token, newOwner, ownerKey, options)
...
LTP.prototype.createObligation = function(rightToken, obligationData, privateKey) {
  ...
  try {
    // Build and sign the correlative obligation token. prepareObligationToken
    // signs inline via _signToken, so no separate proof step is needed.
    var obligationResult = LTPRight.prepareObligationToken(rightToken, obligationData.obligor, obligationData, privateKey)

    if (!obligationResult.success) {
      return {
        success: false,
        error: obligationResult.error
      }
    }

    return {
      success: true,
      obligation: obligationResult.obligationToken,
...
LTP.prototype.createLegalValidityProof = function(token, jurisdiction, nonce) {
  return LTPProof.prepareLegalValidityProof(token, jurisdiction, nonce)
}
