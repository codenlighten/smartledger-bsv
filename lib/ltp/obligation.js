 ...
 * bound to identity (never a key named inside the token's own proof, which a
 * forger controls). did:smartledger embeds the pubkey in the DID itself. Callers
 * may override with options.issuerPublicKey. Returns null when unresolvable —
...
var ObligationToken = {
  ...
  _verifyTokenSignature: function(token, errors, options) {
    try {
      ...
      // Verify the JWS signature against the issuer's key — the real security
      // check. Fail CLOSED: unresolvable key or bad signature => error, never a
      // silent pass.
