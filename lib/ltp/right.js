...
/**
 * Resolve the AUTHORITATIVE signer public key for a token: the issuer's key.
 *
 * The key is bound to the issuer's identity, never taken from a key named inside
 * the token's own proof (which a forger controls). did:smartledger embeds the
 * public key in the DID itself (did:<method>:<pubkeyHex>[:...]), so the issuer DID
 * IS the key. Callers may override with options.issuerPublicKey. Returns null when
 * no key can be resolved — callers MUST fail closed on null.
 */
function _resolveIssuerPublicKey (token, options) {
  options = options || {}
  if (options.issuerPublicKey) {
    return options.issuerPublicKey instanceof bsv.PublicKey
      ? options.issuerPublicKey
      : bsv.PublicKey.fromString(options.issuerPublicKey)
  }
  var did = token && token.issuer
  if (typeof did !== 'string') return null
  try {
    var identifier = did.split('#')[0].split(':')[2] // did:method:<identifier>[:...]
    if (!identifier) return null
    return bsv.PublicKey.fromString(identifier)
  } catch (e) {
    return null
  }
}
...
var RightToken = {
  ...
  /**
   * Build an (unsigned) legal right token structure. Callers that need a signed
   * token use prepareRightToken (signs inline) or add their own proof afterward
   * (e.g. LTP#createRightToken via LTPProof).
   ...
   * @param {PrivateKey} [issuerPrivateKey] - unused here; accepted for signature parity
   * @param {Object} options - Additional options
   * @returns {Object} Unsigned legal right token
   */
  create: function(type, issuerDID, subjectDID, claim, issuerPrivateKey, options) {
    options = options || {}
  ...
  prepareRightToken: function(type, issuerDID, subjectDID, claim, issuerPrivateKey, options) {
    ...
    try {
      var rightToken = this.create(type, issuerDID, subjectDID, claim, issuerPrivateKey, options)
  ...
  prepareRightTokenVerification: function(token, options) {
    ...
    try {
      ...
      this._verifyTokenSignature(token, errors, options)
  ...
  prepareRightTokenTransfer: function(token, newOwnerDID, currentOwnerKey, options) {
    ...
    try {
      ...
      // AUTHORIZATION: bind the signing key to the CURRENT owner. Verifying the
      // token above only proves the ISSUER signed it — NOT that the caller holds
      // it. Without this, prepareRightTokenTransfer(victimToken, attackerDID,
      // attackerKey) would mint a "valid" transfer of a token the attacker does
      // not own. The current owner is token.credentialSubject.id, whose DID embeds
      // the owner's public key (did:key / did:smartledger).
      var ownerDid = token.credentialSubject && token.credentialSubject.id
      var ownerPubKey = _resolveIssuerPublicKey(
        { issuer: ownerDid },
        { issuerPublicKey: options.currentOwnerPublicKey }
      )
      if (!ownerPubKey) {
        return {
          success: false,
          error: 'Cannot resolve current owner public key from token.credentialSubject.id; ' +
            'pass options.currentOwnerPublicKey'
        }
      }
      if (currentOwnerKey.toPublicKey().toString() !== ownerPubKey.toString()) {
        return {
          success: false,
          error: 'Unauthorized transfer: signing key does not match the current owner'
        }
      }
  ...
  /**
   * Verify a token signed by _signToken against a public key.
   *
   * Tokens produced here carry a detached ES256K JWS in proof.jws plus the
   * canonical tokenHash. Verification recomputes the canonical hash (catching
   * tampering of any field), confirms it matches the stored tokenHash, then
   * checks the ECDSA signature embedded in the JWS against the supplied public
   * key. This is the verification counterpart to _signToken and works for
   * right, transfer and obligation tokens alike — all are signed the same way.
   *
   * @param {Object} token - Token with a proof.jws produced by _signToken
   * @param {String|PublicKey} publicKey - Expected signer's public key
   * @returns {Object} { valid, errors, tokenHash, verifiedAt }
   */
  verifyToken: function(token, publicKey) {
    ...
    try {
      ...
      var pubKey = publicKey instanceof bsv.PublicKey
        ? publicKey
        : bsv.PublicKey.fromString(publicKey)

      // Recompute the canonical hash and confirm integrity.
      var canonicalToken = this._canonicalizeToken(token)
      var tokenHash = Hash.sha256(Buffer.from(canonicalToken))

      if (token.tokenHash && token.tokenHash !== tokenHash.toString('hex')) {
        errors.push('Token hash mismatch')
        return { valid: false, errors: errors }
      }

      // Extract the DER signature from the detached JWS (header..signature).
      var parts = token.proof.jws.split('.')
      if (parts.length !== 3 || !parts[2]) {
        errors.push('Malformed JWS')
        return { valid: false, errors: errors }
      }
      var signature = Signature.fromDER(Buffer.from(parts[2], 'base64url'))
  ...
  /**
   * Build a detached compact JWS (RFC 7797) for a token hash + ECDSA signature.
   * The payload segment is intentionally empty (header..signature) because the
   * signed content is the canonical token hash, carried separately as tokenHash.
   * @private
   * @param {Buffer} hash - 32-byte canonical token hash
   * @param {Signature} signature - ECDSA signature over the hash
   * @returns {String} compact detached JWS
   */
   ...
   * @private
   */
  _verifyTokenSignature: function(token, errors, options) {
    try {
      if (!token.proof || !token.proof.jws) {
        errors.push('Missing signature proof')
        return
      ...
      // Verify the JWS signature against the issuer's key. This is the actual
      // security check — the hash comparison above only guards integrity of a
      // caller-supplied field. Fail CLOSED: an unresolvable key or a bad signature
      // must produce an error, never a silent pass.
      var pubKey = _resolveIssuerPublicKey(token, options)
      if (!pubKey) {
        errors.push('Signature not verified: issuer public key could not be ' +
          'resolved from token.issuer; pass options.issuerPublicKey')
        return
      }
      ...
      var ecdsa = new ECDSA()
      ecdsa.hashbuf = expectedHash
      ecdsa.sig = signature
      ecdsa.pubkey = pubKey
      // .verify() returns the instance (truthy); the boolean is on .verified.
      if (ecdsa.verify().verified !== true) {
        errors.push('Signature verification failed')
      }
    } catch (error) {
