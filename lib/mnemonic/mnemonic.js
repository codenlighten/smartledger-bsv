...
/**
 * NFKD-normalise a mnemonic or passphrase, as BIP-39 requires before PBKDF2.
 *
 * Replaces the `unorm` dependency: `String.prototype.normalize` has been in the language
 * since ES6 and this package requires Node >= 20.19, so the shim was redundant.
 *
 * Non-strings return '' so that `Mnemonic.isValid(garbage)` answers false instead of
 * throwing. unorm was inconsistent about this — `[]` and `{}` returned '' while `['a']`
 * and a Buffer threw — and a validity predicate that fails closed for every non-string
 * is both the safer answer and the coherent one.
 */
function nfkd (str) {
  return typeof str === 'string' ? str.normalize('NFKD') : ''
}
...
Mnemonic.prototype.toSeed = function (passphrase) {
  ...
  return pbkdf2(nfkd(this.phrase), nfkd('mnemonic' + passphrase), 2048, 64)
}
...
// Back-reference to the whole library, preserved as public API. Defined as a
// lazy getter rather than `Mnemonic.bsv = require('../../')`: this module is
// loaded while index.js is still initializing, so an eager require would
// capture a half-built namespace. Resolving on first access defers it past
// load and removes the module-init cycle.
Object.defineProperty(Mnemonic, 'bsv', {
  configurable: true,
  enumerable: true,
