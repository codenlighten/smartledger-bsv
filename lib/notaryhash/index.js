...
NotaryHash.verifyBatchInclusion = function (certificate) {
  try {
    ...
    //
    // BRC-220 §On-chain record says the batch tree is RFC 6962 and writes
    // `leaf = SHA256(0x00 ‖ d)` — but that is RFC 6962's own generic notation for the
    // construction, and the spec never binds `d` to a value. `canonicalBytes` appears
    // once in the whole document, in the proofHash definition, and nowhere in the batch
    // text.
    //
    // So this is a choice the implementation had to make, and it is:
    //
    //     leaf = SHA-256(0x00 ‖ proofHash) = SHA-256(0x00 ‖ SHA-256(canonicalBytes))
    //
    // The other reading — d = canonicalBytes — is equally sound cryptographically and
    // produces a DIFFERENT root. A certificate built that way fails inclusion here with
    // no error naming the cause: the fold simply does not reach the root. Anyone building
