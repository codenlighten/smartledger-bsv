...
function resolvePayOutputs (params) {
  ...
  if (params.price != null) {
    var arr = [payOutputFor(params.payTo != null ? params.payTo : params.seller, params.price)]
...
function parseOrdLock (script) {
  try {
    ...
    var payOutputs = splitPayBlob(payBlob).map(function (o) {
      var addr = null
      try { if (o.script.isPublicKeyHashOut()) addr = o.script.toAddress().toString() } catch (e) {}
    ...
    return {
      seller: { pubKeyHash: sellerPKH, address: bsv.Address.fromPublicKeyHash(sellerPKH).toString() },
...
function isOrdLock (script) { return parseOrdLock(script) !== null }
...
function buildPurchaseTx (params) {
  ...
  var ordSats = listing.satoshis != null ? listing.satoshis : 1
  ...
  var fundingTotal = params.funding.reduce(function (a, f) { return a + f.satoshis }, 0)
  ...
  params.funding.forEach(function (f, i) {
    var idx = 1 + i
    var fScript = coerceScript(f.script)
    var sig = H.signInput(tx, f.privateKey, idx, fScript, f.satoshis, P.SIGHASH_ALL_FORKID)
    var us = new Script()
    us.add(Buffer.from(sig))
    us.add(f.privateKey.toPublicKey().toBuffer())
