  ...
  if (fExec && opcodenum >= 0 && opcodenum <= Opcode.OP_PUSHDATA4) {
    ...
    switch (opcodenum) {
      ...
      case Opcode.OP_VERNOTIF:
        // Chronicle: an IF whose condition is "top of stack equals the executing
        // transaction's version", closed by OP_ENDIF.
        //
        // Before Chronicle these are invalid EVEN IN AN UNEXECUTED BRANCH — a rule
        // Bitcoin applies to no other opcode — so the flag check deliberately sits
        // OUTSIDE the fExec guard below, which preserves that.
        if ((this.flags & Interpreter.SCRIPT_ENABLE_CHRONICLE) === 0) {
          this.errstr = 'SCRIPT_ERR_BAD_OPCODE'
          return false
        }
        ...
        if (fExec) {
          ...
          // Popping mirrors OP_IF. The spec describes the comparison but not the
          // stack effect; the OP_VERIF ... OP_ENDIF form it documents is an IF, and
          // an IF that left its condition behind would unbalance every script using
          // it. Pinned by its own test so a correction shows up as a diff.
