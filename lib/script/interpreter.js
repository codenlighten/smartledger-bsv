...
// The post-Genesis replacement: the node's -maxstackmemoryusagepolicy, whose
// default is 100 MB. Consensus is separately configurable and unbounded by
// default, so this is a POLICY ceiling; raise it (or set UNLIMITED) if you are
// deliberately validating against consensus rather than relay rules.
Interpreter.MAX_STACK_MEMORY_USAGE_AFTER_GENESIS = 100 * 1024 * 1024
// The node charges each element its bytes plus the footprint of the container
// holding them, so a stack of many small elements is not free. 32 bytes is
// sizeof(std::vector<uint8_t>) on the platforms the node targets.
