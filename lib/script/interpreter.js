...
/** Failures whose threshold moved with an era, and the cap that moved. */
Interpreter.ERA_SENSITIVE_ERRORS = {
  SCRIPT_ERR_PUSH_SIZE: 'stack element size (capped at 520 bytes pre-Genesis, unbounded after)',
  SCRIPT_ERR_SCRIPT_SIZE: 'total script size (capped at 10,000 bytes pre-Genesis, unbounded after)',
  SCRIPT_ERR_OP_COUNT: 'opcode count (capped at 201 pre-Genesis, unbounded after)',
  SCRIPT_ERR_SCRIPTNUM_OVERFLOW: 'script number width (4 bytes pre-Genesis, 750,000 after Genesis, 32,000,000 after Chronicle)'
}

/** Set false to silence the notice. `eraHint` is still populated. */
Interpreter.eraDiagnostics = process.env.BSV_NO_ERA_HINT !== '1'
...
Interpreter.prototype._eraDiagnose = function (flags) {
  ...
  if ((flags & Interpreter.ERA_FLAGS) !== 0) return
  var cap = Interpreter.ERA_SENSITIVE_ERRORS[this.errstr]
  if (!cap) return
 ...
 *
 * FORKID is deliberately absent here and added by `verify()` only when the input amount
 * is available, since the digest cannot be computed without it.
 *
 * @returns {number}
 */
