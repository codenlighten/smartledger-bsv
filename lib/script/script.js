...
Script.prototype.toBuffer = function () {
  // Sized first and written into one allocation. A locking script can have
  // hundreds of thousands of chunks, and this runs for every signature check
  // over it; a writer that allocates per chunk made that the dominant cost.
  // Writes go through Buffer's checked writers, so a malformed chunk throws
  // exactly where the per-chunk writer threw.
  var chunks = this.chunks
  var size = 0
  var i, chunk, opcodenum
  for (i = 0; i < chunks.length; i++) {
    ...
    if (chunk.buf) {
      if (opcodenum < Opcode.OP_PUSHDATA1) {
        size += bufLength(chunk.buf)
      } else if (opcodenum === Opcode.OP_PUSHDATA1) {
        size += 1 + bufLength(chunk.buf)
      } else if (opcodenum === Opcode.OP_PUSHDATA2) {
        size += 2 + bufLength(chunk.buf)
      } else if (opcodenum === Opcode.OP_PUSHDATA4) {
        size += 4 + bufLength(chunk.buf)
      }
    }
  }
  ...
  for (i = 0; i < chunks.length; i++) {
    chunk = chunks[i]
    opcodenum = chunk.opcodenum
    o = out.writeUInt8(chunk.opcodenum, o)
    if (chunk.buf) {
      if (opcodenum < Opcode.OP_PUSHDATA1) {
        o += chunk.buf.copy(out, o)
      } else if (opcodenum === Opcode.OP_PUSHDATA1) {
        o = out.writeUInt8(chunk.len, o)
        o += chunk.buf.copy(out, o)
      } else if (opcodenum === Opcode.OP_PUSHDATA2) {
        o = out.writeUInt16LE(chunk.len, o)
        o += chunk.buf.copy(out, o)
      } else if (opcodenum === Opcode.OP_PUSHDATA4) {
        o = out.writeUInt32LE(chunk.len, o)
        o += chunk.buf.copy(out, o)
      }
    }
  }
...
// A chunk's data must be a Buffer; BufferWriter#write asserted as much.
function bufLength (buf) {
  if (!Buffer.isBuffer(buf)) throw new assert.AssertionError({ message: 'chunk data must be a Buffer', actual: buf, expected: 'Buffer', operator: 'isBuffer' })
  return buf.length
}
 ...
 *
 * @returns {number|undefined} the opcode, or undefined if the token is not one
 */
function rawOpcodeToken (token) {
  if (!/^0x[0-9a-fA-F]{2}$/.test(token)) return undefined
  var num = parseInt(token.slice(2), 16)
...
Script.prototype._chunkToString = function (chunk, type) {
  ...
  if (!chunk.buf) {
    ...
    // data chunk
    var pushdata = opcodenum === Opcode.OP_PUSHDATA1 ||
      opcodenum === Opcode.OP_PUSHDATA2 ||
      opcodenum === Opcode.OP_PUSHDATA4
    if (!asm && pushdata) {
      str = str + ' ' + Opcode(opcodenum).toString()
...
Script.prototype.findAndDelete = function (script) {
  ...
  // A chunk serializes to bytes starting with its opcode, so only chunks whose
  // opcode is the first byte of `buf` can match. Every other chunk is still
  // serialized when its encoding would throw, so malformed input fails as it
  // always did.
  var first = buf.length ? buf[0] : -1
  for (var i = 0; i < this.chunks.length; i++) {
    var chunk = this.chunks[i]
    if (chunk.opcodenum !== first && serializesCleanly(chunk)) continue
    var buf2 = Script({
      chunks: [chunk]
    }).toBuffer()
    if (buf.equals(buf2)) {
...
function serializesCleanly (chunk) {
  var op = chunk.opcodenum
  if (typeof op !== 'number' || !Number.isInteger(op) || op < 0 || op > 255) return false
  if (!chunk.buf) return true
  if (op > Opcode.OP_PUSHDATA4) return true
  if (!Buffer.isBuffer(chunk.buf)) return false
  var len = chunk.len
  if (op === Opcode.OP_PUSHDATA1) return Number.isInteger(len) && len >= 0 && len <= 0xff
  if (op === Opcode.OP_PUSHDATA2) return Number.isInteger(len) && len >= 0 && len <= 0xffff
  if (op === Opcode.OP_PUSHDATA4) return Number.isInteger(len) && len >= 0 && len <= 0xffffffff
  return true
}
