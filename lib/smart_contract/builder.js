 ...
 * `OP_DUP OP_HASH256 <build-time constant> OP_EQUALVERIFY OP_DROP <pubkey>
 * OP_CHECKSIG` — the hash target is a constant known at build time, so the check
 * binds nothing about the SPENDING transaction and the script reduces to plain
 * P2PK (same flaw as SmartContract.Covenant). Use the enforcing OP_PUSH_TX API
 * instead: `SmartContract.policy()` (declarative), or `SmartContract.PushTx` /
 * `SmartContract.Token` / `SmartContract.valueCovenant`. Construct with
 * `{ allowNonEnforcing: true }` only if you knowingly want the demo behavior.
 *
...
function Builder(privateKey, options) {
  ...
  this.allowNonEnforcing = this.options.allowNonEnforcing === true
...
Builder.prototype._assertEnforcingAllowed = function () {
  ...
  if (!this.allowNonEnforcing) {
    throw new Error(
      ...
      'plain P2PK; it does not constrain the spend). Use SmartContract.policy() ' +
      '(declarative OP_PUSH_TX), or SmartContract.PushTx / Token / valueCovenant for ' +
      'real covenants. To use this demo anyway, construct with { allowNonEnforcing: true }.'
    )
  }
}
...
Builder.prototype.buildUnlockingScript = function(spendingTx, inputIndex, covenantData) {
  ...
  // Add signature
  var covenantScript = Script.fromHex(covenantData.script)
  var signature = sighash.sign(
    spendingTx,
    this.privateKey,
    this.sighashType,
    inputIndex,
    covenantScript,
    new BN(covenantData.satoshis)
  )
...
Builder.prototype.validateSpending = function(spendingTx, covenantUtxo) {
  ...
  var spendingPreimage = sighash.sighashPreimage(
    spendingTx,
    this.sighashType,
    0,
    covenantScript,
...
Builder.prototype._buildMultiFieldCovenant = function(preimageData) {
  ...
  this.validationRules.forEach(function(rule, index) {
    ...
    if (rule.expectedValue) {
      var expectedHash = Hash.sha256sha256(Buffer.from(rule.expectedValue, 'hex'))
...
Builder.prototype._buildHashLockTemplate = function(preimageData, params) {
  ...
  var expectedHash = params.hash || Hash.sha256sha256(preimageData.preimage)
  script.add(expectedHash)
  script.add('OP_EQUALVERIFY')
  script.add('OP_DROP')
