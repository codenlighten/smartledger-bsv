...
class CovenantBuilder {
  ...
  // OP_NIP removes the SECOND item, keeping the top. After `OP_SPLIT` the RIGHT
  // part is on top, so keeping it (dropping the left) requires OP_NIP, not OP_DROP.
  nip() { this.operations.push('OP_NIP'); return this }
  ...
  extractField(fieldName) {
    ...
    if (field.strategy === 'LEFT') {
      ...
      this.size()
        ...
        .sub()
        .split()
        .nip() // OP_SPLIT leaves the RIGHT part on top; drop the LEFT with OP_NIP
        .push(field.length)
        .split()
        .drop() // field is now the LEFT part; drop the trailing remainder
      ...
      if (fieldName === 'scriptLen') {
        ...
        this.push(104) // Fixed left part (4+32+32+36)
          .split()
          .nip() // keep the RIGHT part (tail from offset 104); OP_NIP not OP_DROP
          // For now, assume 1-byte varint
          .push(1)
          .split()
          .drop() // length byte is the LEFT part; drop the trailing remainder
      } else {
