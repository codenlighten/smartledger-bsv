 ...
 * ===================
 *
 * Covenant functionality for Bitcoin SV (BSV): BIP-143 preimage parsing and
 * OP_PUSH_TX covenants that constrain a transaction's own spend.
 *
 * ── ENFORCING COVENANT API (canonical — these actually bind the spend) ──
 * - PushTx: the audited OP_PUSH_TX core (in-script sig gen + OP_CHECKSIG). Now
 *   supports configurable SIGHASH (pushTxCore({sighashType}), assertSighashType).
 * - policy() / DSL: declarative covenant DSL that compiles to a verified OP_PUSH_TX
 *   locking script — the recommended high-level entry point.
 * - Token: stateful ownership-token (NFT) covenants + ordinals (ordinalLock,
 *   transferOrdinal). Authorizers: pluggable single-key / multisig / predicate.
 * - valueCovenant / perpetualCovenant / PELS / Locks: purpose-built primitives.
 * - CovenantHelpers: rawPreimage, verify, fundAndSpend, sighash helpers.
 *
 * ── DEPRECATED, NON-ENFORCING (do NOT use for real covenants) ──
 * - Covenant, Builder: build a constant hash-lock that reduces to plain P2PK and
 *   binds nothing about the spend. Both throw unless constructed with
 *   { allowNonEnforcing: true }. Retained only as workflow/demo scaffolding.
 *
 * ── TOOLING (analysis / debugging, not covenant logic) ──
 * - Preimage, SIGHASH, CovenantBuilder (low-level JS→ASM), OpcodeMap, ScriptUtils,
 *   StackExaminer, ScriptInterpreter, ScriptTester, UTXOGenerator, Debugger.
 */
...
var SmartContract = {
  ...
  CovenantHelpers: require('../covenant/helpers'),
  Locks: require('./locks'),
  PushTx: require('../covenant/pushtx'),
  PELS: require('./pels'),
  Token: require('./token'),
  Authorizers: require('./authorizers'),

  // Declarative covenant DSL + stack debugger (v4.5.0)
  DSL: require('./dsl'),
  Policy: require('./dsl').Policy,
  Debugger: require('./debugger')
}

// Declarative spending policy -> verified OP_PUSH_TX covenant (v4.5.0).
// (Exposes the policy() factory with its .perpetual()/.token() shortcuts.)
SmartContract.policy = SmartContract.DSL.policy
// Step-trace a locking/unlocking pair (returns { ok, err, steps }).
SmartContract.trace = function (unlockingScript, lockingScript, opts) {
  return SmartContract.Debugger.trace(unlockingScript, lockingScript, opts)
}
...
// Stateful ownership token (NFT) locking script. ownerPubKeyHash = HASH160 of the
// owner's public key (SmartContract.Token.ownerId); transfers are ECDSA-authorized.
// `auth` was NOT forwarded here, so a co-signed token built through this alias silently
// came out single-key — the authorizer was accepted and dropped.
SmartContract.ownershipToken = function (fee, ownerPubKeyHash, auth) {
  return SmartContract.Token.ownershipToken(fee, ownerPubKeyHash, auth)
}
...
SmartContract.ordinalLock = function (owner, auth) {
  return SmartContract.Token.ordinalLock(owner, auth)
}
// Transfer a 1-sat (or N-sat) ordinal without burning it — see Token.transferOrdinal.
SmartContract.transferOrdinal = function (signer, newOwnerHash, spend, ordinalSats, lockingScript, opts) {
  return SmartContract.Token.transferOrdinal(signer, newOwnerHash, spend, ordinalSats, lockingScript, opts)
}
...
SmartContract.utils.createCovenantAddress = function (script) {
  ...
  return Address.fromScriptHash(scriptHash)
}
...
// Version information — tracks the package version so the two never drift.
SmartContract.version = require('../../version') // generated; keeps package.json out of bundles
SmartContract.description = 'Smart Contract Framework for Bitcoin SV'
