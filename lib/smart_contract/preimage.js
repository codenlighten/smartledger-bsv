...
// Structural minimum of a BIP-143 preimage, so a too-short buffer fails with an
// HONEST error instead of tripping decodeCompactSize into a misleading
// "8-byte CompactSize" throw. LEFT fixed (nVersion 4 + hashPrevouts 32 +
// hashSequence 32 + outpoint 36 = 104) + smallest scriptLen varint (1) + RIGHT
// fixed (amount 8 + nSequence 4 + hashOutputs 32 + nLocktime 4 + sighash 4 = 52).
Preimage.LEFT_FIXED_LENGTH = 4 + 32 + 32 + 36 // 104
Preimage.RIGHT_FIXED_LENGTH = 8 + 4 + 32 + 4 + 4 // 52
Preimage.MIN_PREIMAGE_LENGTH = Preimage.LEFT_FIXED_LENGTH + 1 + Preimage.RIGHT_FIXED_LENGTH // 157
...
Preimage.prototype.validate = function() {
  ...
  // Length first: a buffer too short to hold the fixed fields can't be
  // field-extracted, so report it without tripping extraction. (For a
  // non-deferred Preimage construction already threw the same error.)
  if (this.preimage.length < Preimage.MIN_PREIMAGE_LENGTH) {
    errors.push('Preimage too short: ' + this.preimage.length +
      ' bytes (BIP-143 preimage requires at least ' + Preimage.MIN_PREIMAGE_LENGTH + ')')
    return {
      valid: false,
      errors: errors,
      warnings: warnings,
...
Preimage.prototype._extractFields = function() {
  ...
  // Fail closed with an honest error before any slice/varint work runs on a
  // buffer too small to hold the fixed BIP-143 fields.
  if (this.preimage.length < Preimage.MIN_PREIMAGE_LENGTH) {
    throw new Error('Preimage too short: ' + this.preimage.length +
      ' bytes (BIP-143 preimage requires at least ' + Preimage.MIN_PREIMAGE_LENGTH + ')')
  }
...
Preimage.prototype._generateRightExtractionASM = function(fieldName, includeComments) {
  ...
  if (includeComments) {
    instructions = instructions.concat([
      ...
      'OP_NIP                    # Drop left (OP_SPLIT leaves right on top): [right_part]',
      targetLen + ' OP_SPLIT             # Extract field: [' + fieldName + ', remaining]',
...
Preimage.prototype._generateLeftExtractionASM = function(fieldName, includeComments) {
  ...
  if (includeComments) {
    instructions = [
      ...
      'OP_NIP                    # Drop prefix (OP_SPLIT leaves remainder on top): [remainder]',
...
Preimage.prototype._generateDynamicExtractionASM = function(parsed, includeComments) {
  ...
  return [
    ...
    'OP_NIP                    # Drop left (OP_SPLIT leaves remainder on top): [remainder]',
...
Preimage.decodeCompactSize = function(buffer, offset) {
  if (offset >= buffer.length) {
    throw new Error('CompactSize read past end of preimage buffer (offset ' +
  ...
  if (firstByte < 0xfd) {
    ...
    if (offset + 3 > buffer.length) {
      throw new Error('Truncated 2-byte CompactSize in preimage at offset ' + offset)
    }
    ...
    if (offset + 5 > buffer.length) {
      throw new Error('Truncated 4-byte CompactSize in preimage at offset ' + offset)
    }
    var value4 = buffer.readUInt32LE(offset + 1)
    return { value: value4, nextOffset: offset + 5, bytes: 5 }
  } else {
    // 0xff marks an 8-byte length (>4 GB) - impossible for a real script length,
    // so this is malformed/misaligned data rather than a value to salvage.
    throw new Error('Invalid CompactSize: 8-byte length prefix (0xff) is not a valid script length')
  }
}
...
Preimage.fromTransaction = function(transaction, inputIndex, subscript, satoshis, sighashType) {
  var preimageBuffer = sighash.sighashPreimage(
    transaction,
    sighashType,
    inputIndex,
    subscript,
...
Preimage.createExample = function(sighashType) {
  ...
  var subscript = Script.fromHex(utxo.script)
