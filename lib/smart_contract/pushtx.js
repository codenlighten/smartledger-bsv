...
// the signature.
var SIGHASH_ALL_FORKID = Signature.SIGHASH_ALL | Signature.SIGHASH_FORKID // 0x41
var SIGHASH_SINGLE_FORKID = Signature.SIGHASH_SINGLE | Signature.SIGHASH_FORKID // 0x43
var SIGHASH_SINGLE_ANYONECANPAY_FORKID =
...
var SIGHASH_ALL_ANYONECANPAY_FORKID =
...
function pushTxCore (script, opts) {
  var sighashType = (opts && opts.sighashType) || SIGHASH
...
function assertSighashType (script, sighashType) {
  var word = Buffer.from([sighashType & 0xff, 0x00, 0x00, 0x00])
...
function sFromPreimage (preimage) {
  ...
  // NOT s.toBuffer('be', 32): lib/crypto/bn.js REPLACES toBuffer with an
  // options-object form, so 'be' would land in `opts`, opts.size would be
  // undefined and the 32 ignored — yielding a natural-length buffer.
  //
  // That truncation is currently UNREACHABLE, and the reason is worth writing
  // down because it is not obvious: z is filtered to a leading byte of
  // 0x01..0x7f above, so z is in [2**248, 2**255); adding Gx (~0.476 * 2**256)
  // never wraps; and the low-S check caps the result at n/2. s therefore lands
  // in a narrow band with a leading byte of 0x7a..0x7f and is always 32 bytes.
  //
  // The call is still wrong, and worth fixing, because that safety is an
  // accident of two filters that have nothing to do with buffer length. Widen
  // the MINIMALDATA range or drop low-S and the truncation becomes reachable —
  // silently, inside a covenant, where DER_PREFIX declares s to be exactly 32
  // bytes and a short one makes the spend invalid.
  var sBE = s.toBuffer({ size: 32 })
...
function grind (spend, inputIndex, lockingScript, satoshis, opts) {
  ...
  opts = opts || {}
  var maxTries = opts.maxTries || 5000
  var start = opts.start || 0
  var field = opts.field || 'nLockTime'
  var sighashType = opts.sighashType || SIGHASH
  if (field !== 'nLockTime' && field !== 'sequence') {
    throw new Error("grind field must be 'nLockTime' or 'sequence'")
  }
  var input = spend.inputs[inputIndex]
  for (var i = 0; i < maxTries; i++) {
    var nonce = start + i
    if (field === 'nLockTime') spend.nLockTime = nonce
    else input.sequenceNumber = 0xfffffffe - nonce // stay < 0xffffffff (non-final)
    var preimage = H.rawPreimage(spend, inputIndex, lockingScript, satoshis, sighashType)
    // Length-checked, not truthy: grind's contract is a FIXED-length DER
    // signature, and a short s would be truthy. Belt-and-braces alongside the
    // toBuffer fix above — this is the check that would catch it if the
    // upstream filters that currently make a short s impossible ever change.
    var sBE = sFromPreimage(preimage)
    if (sBE && sBE.length === 32) return { preimage: preimage, tries: i + 1, field: field, nonce: nonce }
  }
...
module.exports = {
  ...
  assertSighashAll: assertSighashAll,
  assertSighashType: assertSighashType,
  SIGHASH_ALL_FORKID: SIGHASH_ALL_FORKID,
  SIGHASH_SINGLE_FORKID: SIGHASH_SINGLE_FORKID,
  SIGHASH_SINGLE_ANYONECANPAY_FORKID: SIGHASH_SINGLE_ANYONECANPAY_FORKID,
  SIGHASH_ALL_ANYONECANPAY_FORKID: SIGHASH_ALL_ANYONECANPAY_FORKID,
