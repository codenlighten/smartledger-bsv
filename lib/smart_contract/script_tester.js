...
function ScriptTester(options) {
  ...
  this.flags = this.options.flags || Interpreter.SCRIPT_ENABLE_SIGHASH_FORKID
...
ScriptTester.prototype.execute = function(config) {
  ...
  var satoshisBN = new BN(this.satoshis)
...
ScriptTester.prototype.debug = function(config) {
  ...
  try {
    for (var i = 0; i < chunks.length; i++) {
      var chunk = chunks[i]
      var opname = Opcode.reverseMap[chunk.opcodenum] || 'PUSH'
  ...
  try {
    if (isHex) {
      ...
      if (hexLength >= 180 && hexLength <= 250) {
        ...
        return new Script().add(Buffer.from(trimmed, 'hex'))
      } else {
        // Normal hex script
        return Script.fromHex(trimmed)
      }
    } else {
      return Script.fromASM(trimmed)
    }
  } catch (error) {
...
ScriptTester.prototype._createCombinedScript = function(config) {
  ...
  return Script.fromBuffer(combinedBuffer)
}
