...
function asmToScript(asmString) {
  try {
    return Script.fromASM(asmString)
  } catch (error) {
...
function validateScript(script) {
  try {
    ...
    if (typeof script === 'string') {
      scriptObj = Script.fromASM(script)
    } else if (Buffer.isBuffer(script)) {
      scriptObj = Script.fromBuffer(script)
    } else if (script && typeof script.toBuffer === 'function') {
      scriptObj = script
    } else {
...
function validateSyntax(asmString) {
  ...
  for (let i = 0; i < operations.length; i++) {
    ...
    if (op.startsWith('OP_') && !Opcode.map[op]) {
...
function estimateScriptSize(script) {
  try {
    ...
    if (typeof script === 'string') {
      ...
      scriptObj = Script.fromASM(script.join(' '))
    } else if (Buffer.isBuffer(script)) {
...
function optimizeScript(script) {
  try {
    ...
    try {
      optimizedScript = Script.fromASM(optimizedASM)
    } catch (err) {
...
function compareScripts(scriptA, scriptB) {
  try {
    ...
    if (typeof scriptA === 'string') {
      bufferA = Script.fromASM(scriptA).toBuffer()
    ...
    if (typeof scriptB === 'string') {
      bufferB = Script.fromASM(scriptB).toBuffer()
