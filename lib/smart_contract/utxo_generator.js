...
function UTXOGenerator(options) {
  ...
  this.network = this.options.network || Networks.mainnet
  ...
  // SmartUTXO deprecation warning (the warning is for downstream
  // consumers who reach for SmartUTXO themselves, not for our own
...
UTXOGenerator.prototype.generateKeypair = function(label) {
  ...
  var privateKey = PrivateKey.fromRandom(this.network)
...
UTXOGenerator.prototype._createSingleUTXO = function(config) {
  ...
  switch (config.scriptType) {
    case 'P2PKH':
      script = Script.buildPublicKeyHashOut(config.keypair.address)
      scriptHex = script.toHex()
      break
      
    case 'P2SH':
      // Create a simple multisig for P2SH example
      var redeemScript = Script.buildMultisigOut([config.keypair.publicKey], 1)
      script = Script.buildScriptHashOut(redeemScript)
      scriptHex = script.toHex()
      break
      
    case 'CUSTOM':
      // Allow custom script injection
      script = config.customScript || Script.buildPublicKeyHashOut(config.keypair.address)
      scriptHex = script.toHex()
      break
