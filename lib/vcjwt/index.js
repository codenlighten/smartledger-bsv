...
async function verifyVcJwt (jwt, opts) {
  ...
  try {
    ...
    // A token with no `exp` never expires. Callers that require bounded-lifetime
    // credentials should pass requireExpiration:true to reject exp-less tokens.
    if (opts.requireExpiration && !payload.exp) {
      return { valid: false, error: 'JWT missing required exp claim', header: header, payload: payload }
    }
