...
describe('Script text round trip', function () {
  ...
  it('round-trips every push encoding: exactly through toString, minimally through ASM', function () {
    ...
    var sizes = [0, 1, 75, 76, 255, 256, 300]
    var encodings = [
      ...
      function (d) { var h = Buffer.alloc(3); h[0] = 0x4d; h.writeUInt16LE(d.length, 1); return Buffer.concat([h, d]) },
      function (d) { var h = Buffer.alloc(5); h[0] = 0x4e; h.writeUInt32LE(d.length, 1); return Buffer.concat([h, d]) }
    ]
    var checked = 0
    sizes.forEach(function (n) {
      encodings.forEach(function (enc) {
        var push = enc(Buffer.alloc(n, 0x5a))
        if (!push) return
        var buf = Buffer.concat([Buffer.from([0x51]), push, Buffer.from([0x87])])
        var s = Script.fromBuffer(buf)
